...
\begin{enumerate}[label=\arabic*.]
  \item Regarding the fixed CPU--memory demand-ratio assumption, we added CPU--memory demand-ratio measurements for open-source benchmarks in the Background and added explanation about the model scope and boundary cases in the Discussion, as suggested by Reviewers 1, 2, and 3.
  ...
  \item We added performance analyses under bursty workload and high offered load in the Evaluation, as suggested by Reviewer 1 and 2.
...
To facilitate reviewers to quickly locate the revised content, we attach the complete revised paper at the end of this PDF and highlight the modified parts in {\color{blue}BLUE}. We also upload the highlighted revised version in the “Supplementary Material for Review”.
...
\end{reviewcomment}
\response{3.2} Thank you for your comment. We first would like to clarify that the phrase ``accurate demand estimates'' in the initial submission was an overstatement, and we have revised the corresponding wording. What we intended to express is that COSMOS determines instance resource allocation based on the request resource demand, rather than on user-provided resource configurations, which are often over-provisioned. In our test environment and benchmarks, such request-level resource demand can be derived from request input data.

Request-level resource-demand prediction itself is outside the scope of this paper. COSMOS focuses on using known function-level resource-demand characteristics to allocate resources for requests with intra-function heterogeneous demands, thereby avoiding frequent cold starts and structural resource fragmentation. The performance advantage of COSMOS over the baselines comes from reducing cold starts and resource fragmentation, not from a more accurate request-demand predictor; the latter is more closely related to resource-overcommitment topics. In fact, all evaluated systems in our experiments use the same request-demand prediction method (except for Golgi who has its own overcommitment strategy), and COSMOS differs from the baselines only in request routing and task scheduling. Therefore, we did not treat request-demand estimation accuracy as a separate robustness dimension. We clarified this boundary in the Discussion:
...
Since COSMOS is the only evaluated system that uses predictions of future per-function load to guide scheduling decisions, we additionally report the performance difference between COSMOS and the baselines under different levels of function-level load-prediction error. These results show that COSMOS's advantage over the baselines does not rely on exact future-load prediction:
...
For the CPU-to-memory ratio assumption mentioned in this comment, we added an explicit scope analysis in the Discussion. It explains why the assumption is consistent with the serverless function-decomposition model, how single-resource-dominant functions can be represented conservatively, and why complex functions whose profiles change qualitatively across requests are not directly supported. We also added an empirical characterization of the resource-demand profiles of the open-source serverless benchmarks used in this paper:
