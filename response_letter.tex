...
\begin{enumerate}[label=\arabic*.]
  \item We strengthened the validation and scope of the stable CPU-to-memory demand-ratio model in the Background, Design, and Discussion, and reported per-function $R^2$ and ratio CV across inputs.
  \item We clarified node-level contention and eviction amplification, and added analyses of demand relocation, scale-up-triggered eviction, pod-eviction rate, cold starts, tail latency, and instance lifetime.
  \item We clarified that beneficial swaps are internal routing-plan operations rather than runtime pod migrations or evictions, and evaluated their observable system-level effects through net demand relocation and instance-churn metrics.
  \item We sharpened the technical novelty of \sysname and its distinction from Golgi, and expanded the component breakdown using the Routing* and Scheduling* ablations.
  \item We expanded the evaluation with a bursty workload, prediction errors, Golgi and Golgi-NoCC, generality across workload and cluster configurations, and scheduler scalability up to 1024 workers.
  \item We added a Discussion of the workload regimes in which vertical scaling is useful, the applicability of size-specific prewarmed pools, and the limitations and possible extensions of the current demand model.
  \item We improved presentation and reproducibility by reporting resource configurations in the online supplement, clarifying experimental settings and evaluation boundaries, and correcting the reported typographical errors.
\end{enumerate}

In the following pages, we provide detailed responses to the comments raised by the three reviewers. For ease of review, we place each comment in a gray box and provide the corresponding response below it. Section, figure, and table references in this letter refer to the revised manuscript unless explicitly stated otherwise.

The final response package will also include the complete revised manuscript with all modified text highlighted in blue. We highly appreciate your time and effort in reassessing our manuscript.
...
\end{reviewcomment}
\response{1.1} Thank you for your comment. We agree that the current modeling approach is not suitable for the transformer-style example raised by the reviewer, where one request may be dominated by a prefill phase and another by a decoding phase. Such a function has resource-demand patterns that are tied to request type because a single function contains multiple execution phases with qualitatively different resource profiles. We added a discussion of this limitation in Section~VII:

\begin{revisionquote}
``The current model does not directly support complex functions whose resource consumption pattern changes qualitatively across requests. For such cases, a natural extension is to classify requests into several virtual functions, each with a single resource profile, and then apply the existing routing model to those virtual functions. A more fundamental approach is to refactor such functions by splitting branching or multi-stage processing logic into more independent tasks, making their resource behavior easier for COSMOS or other resource schedulers to model stably. We leave these extensions to future work.''
\end{revisionquote}

For most fine-grained serverless functions, however, the fixed CPU-to-memory demand-ratio model remains a reasonable abstraction: requests often vary in demand magnitude while preserving a stable resource profile. We added empirical validation and analysis in Section~II-B (using peak memory as the memory-demand metric, since it determines the actual resource allocation required for a function instance):
...
We also analyzed the two streaming functions in Figure~2, whose peak memory remains nearly constant while CPU consumption grows with input size. These functions can still be represented conservatively, although specialized support would require an extended model:
...
\begin{revisionquote}
``Single-resource-dominant streaming functions, such as video-processing and uploader in Figure~2, can still be represented within the current model by assigning a conservative fixed resource ratio. This may over-allocate the less variable resource but preserves normal system operation. Providing specialized support for streaming functions would require extending both the demand model and the SwapCapacity routine to allow a function's demand ratio to vary with request size.''
\end{revisionquote}
...
\end{reviewcomment}
\response{1.2} Thank you for your comment. We added experiments in Section~VI-C to measure system behavior under a workload spike, eviction frequency, eviction pressure caused by in-place scale-up, and the resulting impact on instance lifetime:
...
Finally, we examine instance lifetime to evaluate the cumulative impact of eviction and churn. The results show that COSMOS shifts instance lifetimes toward longer durations:
...
Because COSMOS uses heuristic online routing and scheduling, deriving a precise workload-independent bound on the impact of contention is difficult. However, the added measurements above evaluate the observable consequences of contention---cold starts, tail latency, pod eviction, and instance lifetime---and show experimentally that COSMOS maintains a low eviction frequency, mitigates burst-induced latency spikes, and preserves longer-lived warm instances under the evaluated bursty scenarios.
...
\end{reviewcomment}
\response{2.1} Thank you for your comment. We added empirical validation in Section~II-B to examine whether the CPU-to-memory ratio remains stable within the same function as request inputs change. The results show that most evaluated functions satisfy this modeling assumption: eight of the ten SeBS benchmarks exhibit a stable function-specific CPU-to-peak-memory profile, even though their demand magnitude changes across inputs:
...
We also added a discussion of the cases that are not fully captured by this model, including single-resource-dominant streaming functions and complex functions whose resource behavior changes qualitatively across request types:
...
uploader in Figure~2, can still be represented
...
\end{reviewcomment}
\response{2.2} Thank you for your suggestion. We agree that the individual mechanisms used by COSMOS, such as request routing, resource-aware placement, and vertical scaling, are not entirely new in isolation. We revised the manuscript to clarify that the technical novelty of COSMOS lies in modeling function-level and request-level resource-demand heterogeneity in serverless workloads, and proactively using this model to guide resource allocation and load partitioning. We clarified this point in Section~I:

\begin{revisionquote}
``To overcome these inefficiencies, we design COSMOS to proactively model and exploit two complementary sources of heterogeneity: resource-demand diversity across functions and dynamic demand variation within each function. At the inter-node level, we introduce complementary swarming, a function request routing strategy that mitigates resource stranding by co-locating functions with opposing resource requirements while preserving locality. At the intra-node level, we design a monotonic scale-up request scheduling policy that reorders the request queue for a function to enforce monotonic resource adjustment that avoids the high overhead of instance restarts. Together, these techniques align resource allocation with modeled demand and significantly improve cluster utilization.''
\end{revisionquote}

Compared with COSMOS, Golgi also recognizes function-level and request-level resource-demand heterogeneity, but it uses a reactive scheduling design: it monitors low-level runtime metrics and uses performance prediction to route requests between instances with different resource quotas, thereby avoiding performance loss from resource contention. 
At the cluster level, Golgi favors recently used instances to reduce idle instance footprint. However, this strategy does not explicitly exploit resource complementarity across functions or the instance-reuse benefit of concentrating each function's load on fewer nodes, which is one of the main performance advantages of COSMOS. We clarified the distinction in the experimental setup and Related Work sections:
...
\begin{revisionquote}
``Golgi~[43] uses overcommitment to maintain differently sized instances of each function and favors the most recently accessed instance when routing. However, these approaches do not explicitly consider the instance-reuse benefit of concentrating a function's load on fewer nodes. ''
\end{revisionquote}
...
\evidencefigure{figures/fig_overhead_combined.pdf}{Revised Figure 18: Routing-plan generation and task-scheduling overhead under different cluster sizes and application workloads.}
...
Second, to more clearly investigate how COSMOS behaves when a sudden load shift makes a periodically generated routing plan stale, we set the routing-plan update interval to five minutes and evaluate how the system handles the bursty episode shown in Figure~11.
...
\end{reviewcomment}
\response{3.1} Thank you for your comment. We agree that size classes can provide a request-level resource-allocation effect similar to COSMOS's monotonic scaling algorithm in some settings. The benefit of monotonic scaling is that it provides more continuous and fine-grained resource adjustment for dynamically changing resource demands, while avoiding the need to design function-specific class-sizing schemes and avoiding the within-class over-provisioning that commonly arises with size classes. We added the following discussion in Section~VII to clarify this distinction and characterize the regime where size-specific pools are applicable:

\begin{revisionquote}
``The central challenge in predictive horizontal scaling with size-specific prewarmed pools is determining function-specific boundaries that partition requests into resource-size classes. Coarse-grained classes retain substantial within-class demand variation and therefore require over-provisioning, whereas fine-grained classes fragment warm capacity across pools and reduce instance-reuse opportunities. Monotonic vertical scaling avoids this granularity trade-off by allowing a warm instance to serve requests with different resource demands without predefined size boundaries. We acknowledge that size-specific pools can be effective when a function serves only a few stable request types, or when its invocation volume is sufficiently high to maintain high reuse within every fine-grained pool. Outside these regimes, vertical scaling provides a more flexible way to accommodate heterogeneous request demands.''
\end{revisionquote}
...
\end{reviewcomment}
\response{3.2} Thank you for your comment. We added empirical validation and an explicit model boundary in Sections~II-B and VII. The revised Discussion now distinguishes stable demand-magnitude variation, conservatively modeled streaming functions, and complex request patterns that would require virtual functions or refactoring. The prototype derives estimates from benchmark input characteristics; a deployment can supply estimates from an independent predictor because prediction is orthogonal to the placement algorithm. Section~VI-C now evaluates noisy and biased estimates:
...
\begin{revisionquote}
``COSMOS models each function with a stable CPU-to-memory demand ratio and a variable demand magnitude. This model is broadly applicable to fine-grained serverless functions because many of them are single-purpose and exhibit stable CPU-to-peak-memory profiles, as shown in Figure~2.''
\end{revisionquote}
...
\begin{revisionquote}
``The current model does not directly support complex functions whose resource consumption pattern changes qualitatively across requests. For such cases, a natural extension is to classify requests into several virtual functions, each with a single resource profile, and then apply the existing routing model to those virtual functions. \ldots{} We leave these extensions to future work.''
\end{revisionquote}

\begin{revisionquote}
``For a $\pm x\%$ random-error setting, we independently multiply each function's predicted load at every routing update by $1+\epsilon$, where $\epsilon$ is uniformly sampled from $[-x,x]$. For a $\pm y\%$ systematic-bias setting, every predicted function load is multiplied by the fixed factor $1+y$ or $1-y$, respectively. \ldots{} Nevertheless, all prediction-error variants retain a clear and consistent advantage over the baselines, indicating that COSMOS's overall performance gains are robust to imperfect demand estimates.''
\end{revisionquote}

The error experiment evaluates imperfect demand-magnitude estimates under a stable profile; it does not claim to cover qualitative CPU-to-memory profile switches, which are now identified as outside the current model and as a target for the virtual-function or refactoring extensions discussed above.
