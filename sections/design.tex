...
$\alpha_j^{\text{swap}}$ of $F_a$ demand that can be hosted by beneficially
swapping out fractions of existing assignments. A swap is considered beneficial only if it increases 
...
the admissible swap capacity $\alpha^{\text{swap}}$ together with the internal
swap list $\mathcal{S}$.

The subroutine proceeds in four logical steps. First, it identifies the dominant 
idle resource\textemdash CPU or memory\textemdash since after using the node's free capacity (line~11 
...
they are sorted in descending order (more compute-intensive ones first). This ordering 
...
and the corresponding internal swap list $\mathcal{S}$ are returned to the main loop 
in Algorithm~\ref{alg:placement}.
