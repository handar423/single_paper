...
The complementary swarming routing algorithm addresses the scheduling challenge 
...
the capacity vector. Furthermore, we define the \textbf{overall resource utilization} 
...
balances both resource dimensions. An optimization step in the algorithm is considered 
beneficial if it increases \(U(N_j)\).
...
determines request routing rule: when a function's workload is split across multiple 
...
functions that already hold many instances) and promotes fair resource sharing. 
...
\begin{itemize}
    ...
    is narrow; consequently, an instance that starts with $t_{\text{first}}$ will 
    require little additional scaling to process the remaining requests. A small 
