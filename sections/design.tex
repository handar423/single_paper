...
serverless function and request: complementary swarming request dispatching at the 
inter-node level and monotonic scale-up request scheduling at the intra-node level.

\subsection{Complementary Swarming Request Dispatching}
\label{sec:design:placement}

The complementary swarming dispatching algorithm addresses the scheduling challenge 
...
A precise formulation of the dispatching problem requires clear representations of 
...
complementary dispatching and proportional workload distribution simultaneously.

{
% \setlength{\belowdisplayskip}{-140pt}
\begin{algorithm}[t]
    \caption{\mbox{Complementary swarming dispatching.}}
    \noindent \textbf{Data:}
    \begin{algorithmic}[1]
    ...
    \end{itemize}
    \Ensure Optimized dispatching map $\mathcal{P}: \mathcal{F} \times [0,1] \to \mathcal{N}$.
...
\subsubsection{Complementary Swarming Dispatching Algorithm}
\label{subsec:placement-algorithm}

Algorithm~\ref{alg:placement} presents the complementary swarming dispatching 
...
profiles $\langle r_i, \mathbf{V}_i, D_i \rangle$. The algorithm outputs a dispatching 
map $\mathcal{P}: \mathcal{F} \times [0,1] \to \mathcal{N}$. This mapping 
determines request dispatch: when a function's workload is split across multiple 
...
subsequent dispatching operates under a balanced supply-demand premise~(Line~3--4). 
...
\textbf{Step 3: Fine-Grained Dispatching Generation.}
After the assignment matrix $\mathbf{L}$ is determined, the algorithm converts 
the proportional assignments into a precise dispatching map $\mathcal{P}$ 
...
\parabf{Request Dispatching.} At runtime, the interval $[0, 1]$ is concretely mapped 
...
its percentile $p$. The request is then dispatched to the node $N_j$ for which 
...
the function with the highest priority and attempts to dispatch the head request of 
its current~queue.
