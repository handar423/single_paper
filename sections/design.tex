...
\begin{algorithm}[t]
    ...
    \While{$\exists \; F_i \text{ s.t. } R[i] > 0$} \Comment{Incomplete mapping}
        ...
        \For{$N_j \in \mathcal{N}$}
            ...
            \State $\Delta U_j \gets \text{UtilGain}(F_a, \alpha_j^{\text{max}}, \mathcal{E}_j, \mathbf{C}_j)$
        \EndFor
...
function swapping, and a list $\mathcal{E}_j$ of evicted functions, each with the 
...
the algorithm reduces the evicted fraction $\beta_k$ in $\mathbf{L}[j, k]$ and 
...
and the corresponding eviction list $\mathcal{E}$ are returned to the main loop 
...
of functions that need to be re-routed in each iteration, which in turn minimizes 
instance migration, thereby reducing scheduling churn and preserving warm instance 
...
evaluates eviction. It selects candidate instances according to a per-sandbox 
