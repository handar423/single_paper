...
benchmarking suite~\cite{copik2021sebs, benchmark_10} to evaluate the performance of \sysname 
across diverse workloads. Table\ref{tab:setup} summarizes the basic distribution 
of these benchmarks. The selected benchmarks 
...
In our testdeb \junyi{typo?}, the average memory consumption of these benchmarks ranges from 
...
the average execution times range from 0.15 to 21.5 seconds for these applications. \junyi{can we include these nubmers for each workload in the table?}
...
\begin{table}[ht]
    \vspace{-10pt}
    \caption{Performance comparison under multiple workflows.\label{tab:setup} \junyi{name of this table is not very accurate}}
    ...
    \begin{tabularx}{0.47\textwidth}{ 
        ...
        \toprule
        \textbf{Workload} & \textbf{Type} & \textbf{Distribution} \\
        \hline 
        \hline 
        ...
        \hline
        image-recognition & \raisebox{-5pt}{ML} & Performing image classification using ResNet and PyTorch. \\
        \hline
...
knative_scaling_window}, the scaling interval is set to 1 minute. \junyi{is the generation method standard? If so, can we add some citations: we follow the same methodology as xxxx to generate the workload.}

To simulate the real-world characteristic where the resource demand of a serverless 
function follows a normal distribution across multiple executions \junyi{We said there are 4 different distributions earlier in the paper. Isn't it reasonable to have all of them?}, we first 
...
\parabf{Cluster configuration.} Our experiments are conducted on CloudLab \todo{cite} using 
...
CPU and memory utilization, achieving relative improvements of 43.61\%$\sim$55.53\% \junyi{use -- instead of $\sim$}
...
First, the complementary swarming instance placement strategy enables individual 
...
waste caused by load-resource mismatch. Second, the monotonic scale-up task 
...
This scaling trend can be attributed to the complementary swarming instance placement. With 
...
\begin{figure*}[ht]
    ...
    \caption{Performance comparison under different workload distributions. (a) CPU Utilization. (b) Memory Utilization. (c) Task throughput. \junyi{looks weird. either remove the blank between uniform and zipf or add another set of bars (zipf of a different skewness)}}
    \vspace{-15pt}
...
increases instance reuse opportunities. Second, the monotonic scale-up task 
...
\begin{itemize}
    \item \textbf{Placement*}: This variant uses only the complementary swarming 
...
complementary swarming instance placement strategy alone can effectively enhance 
scheduling performance through load partitioning. Furthermore, it requires integration 
with the monotonic scale-up task scheduling policy for fine-grained, 
...
When integrated with the complementary swarming instance placement strategy 
