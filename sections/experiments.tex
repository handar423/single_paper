...
state-of-the-art systems\textemdash Palette~\cite{abdi2023palette} and 
FaaSCache~\cite{FaaSCache}\textemdash in \S\ref{sec:experiments_comparsion}. Next, 
...
in systems and performance~analysis~\cite{Gallager2013Stochastic}. 
...
ensure fair comparison and minimize engineering artifacts, we reimplement both 
baselines in our prototype environment. Other components of the prototype system 
are kept identical across all evaluated systems.
...
To verify the potential concern of task starvation, we examine the impact on 
...
First, the complementary swarming request routing enables individual 
...
\begin{table}[t]
    \vspace{-5pt}
    \caption{Tail latency comparison between \sysname and baselines.\label{tab:latency}}
...
In addition to the default resource distribution, 
...
while ensuring that the two most frequently invoked functions\textemdash randomly chosen as 
\textbf{thumbnailer} and \textbf{video-processing}\textemdash collectively account for 80\% 
of all invocations.
...
\parabf{Performance analysis.} To further illustrate the reasons behind \sysname's 
...
during task processing in the default evaluation setting. We categorize CPU and 
memory resources into the following 
four types: 
...
complementary swarming routing strategy alone can effectively enhance performance. 
Furthermore, it requires integration with the monotonic scale-up scheduling policy 
for fine-grained, node-internal resource management to achieve better scheduling efficiency.
