...
\begin{table*}[ht]
    \vspace{-10pt}
    \caption{\yunshan{Benchmark characteristics: resource profiles and execution times.}\label{tab:setup}}
    \centering
    \begin{tabular}{r c c c c c c}
        ...
        video-processing & Multimedia   & 1.5 & 4.0 & 11.0 & 0.38 & Adding watermarks and generates GIFs from video files. \\
        \bottomrule
    \end{tabular}
\end{table*}
...
The invocation rate of each trace is proportionally scaled so that the aggregated 
...
relative difference of less than 2\%). \yunshan{Since the original trace data is 
recorded at minute-level granularity, we model intra-minute request arrivals using 
a Poisson process, a standard model for independent random events in systems and 
performance analysis~\cite{Gallager2013Stochastic}.} In alignment with common 
industrial practice~\cite{aws_scaling_window, knative_scaling_window}, the 
scaling interval is set to 1 minute.

\yunshan{To accurately model real-world serverless functions, we first analyze 
the resource assumption distribution in the dataset~\cite{ShahradFGCBCLTR20}. 
Using both the Shapiro-Wilk~\cite{SHAPIRO1965} and Kolmogorov-Smirnov 
normality~\cite{f4feef4d-12ae-314a-a10b-d1f2768e819a} tests, we find that over 
95\% of functions exhibit memory usage patterns that do not significantly deviate 
from a normal distribution (p-value $>$ 0.05). Based on this empirical observation, 
we model the per-function resource demand as normally distributed across function requests.}
For our evaluation, we randomly select ten functions from the dataset and perform 
Gaussian fitting to obtain the variance of their memory usage. We then adjust 
the input data size and parameters of each benchmark so that its actual resource 
requirements match the sampled variance. Since the dataset primarily reflects 
memory demand, we assume the CPU allocation proportionally with the fixed 
CPU-to-memory ratio for each benchmark.

\parabf{Cluster configuration.} Our experiments are conducted on CloudLab~\cite{cloudlab} using 
a cluster consisting of 9 c6620 instances (56 cores in total, 2$\times$ Intel Xeon Gold 5512U 
...
and FaaSCache~\cite{FaaSCache}, which optimize cluster efficiency from 
complementary perspectives. Palette enhances spatial locality by allowing 
...
Inspired by caching systems, FaaSCache employs a Greedy-Dual keep-alive 
...
across all systems during evaluation (see prototype implementation details 
...
First, the complementary swarming request dispatching strategy enables individual 
...
This scaling trend can be attributed to the complementary swarming dispatching. With 
...
dispatching concentrates a narrower set of functions on each node, which 
...
\begin{itemize}
    \item \textbf{Dispatching*}: This variant uses only the complementary swarming 
    request dispatching strategy, while tasks within each node are scheduled using 
...
As shown, the performance of the Dispatching* version lies between the baselines 
...
complementary swarming dispatching strategy alone can effectively enhance 
...
tasks with lower resource demands, shorter execution times, and higher execution 
frequencies, without explicitly optimizing for overall server utilization.

When integrated with the complementary swarming dispatching strategy 
...
\begin{figure}[t]
    ...
    \caption{The average duration for \sysname to generate a dispatching strategy.}
...
Figure~\ref{fig:overhead_placement} shows the average duration for generating cluster-wide 
request dispatching strategies under different cluster sizes. It can be observed 
...
the overall load proportionally to the cluster size before computing the dispatching 
...
the scheduler can generate dispatching strategies promptly, even in large-scale 
...
task under different invocation frequencies. It should be noted that 
dispatching strategy generation is performed by a separate background process within 
the scheduler. The main scheduling process only receives the resulting dispatching 
