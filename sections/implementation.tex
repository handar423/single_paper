...
i) The routing plan generator runs as a periodic cluster-level service (currently every minute). It executes the complementary swarming routing algorithm to compute the optimized request routing plan, which is provided to the request router.
ii) The request router operates as a separate cluster-level process. It assigns each incoming request to a worker node according to the current routing plan received from the generator.
