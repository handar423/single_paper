...
\textbf{a)} The cluster-level workload-splitting manager runs periodically (currently 
every 5 minutes), executes the workload-splitting-based instance placement algorithm, 
and passes the resulting to the task dispatcher process. \textbf{b)} The cluster 
task dispatcher, operating in a separate process, assigns each incoming task to 
a worker node according to the placement plan. \textbf{c)} The worker-level task 
...
\textbf{d)} The system monitor collects real-time resource usage, records per-function 
