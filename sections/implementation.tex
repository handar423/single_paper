...
i) The dispatching strategy generator runs as a periodic cluster-level service (currently every minute). It executes the complementary swarming dispatching algorithm to compute the optimal request dispatching map and provides this strategy to the request dispatcher.
ii) The request dispatcher operates as a separate cluster-level process. It assigns each incoming request to a worker node by directly applying the current dispatching strategy received from the generator.
iii) The worker scheduler runs locally on each worker node. It maintains per-function task queues, enforces the monotonic scale-up scheduling policy to determine execution order, and triggers in-place pod scaling when required.
