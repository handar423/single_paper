...
Kubernetes~\cite{k8s} for instance management, Minio~\cite{redis} for function 
...
\textbf{i)} The cluster-level workload-splitting manager runs periodically (currently 
every 5 minutes), executes the workload-splitting-based instance placement algorithm, 
and passes the resulting to the task dispatcher process. \textbf{ii)} The cluster 
task dispatcher, operating in a separate process, assigns each incoming task to 
a worker node according to the placement plan. \textbf{iii)} The worker-level task 
scheduler maintains local task queues on each worker and decides when and which 
task to execute based on the vertical-scaling-based task scheduling policy. 
\textbf{iv)} The system monitor collects real-time resource usage, records per-function 
