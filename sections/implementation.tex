...
\parabf{Instance manager.} We use containers as serverless function instances
managed through Kubernetes. Each function instance is exposed
...
and multi-coroutine architecture. Since some benchmarks require reading and
writing external data during execution, we additionally deploy a MinIO service
...
iv) The system monitor collects real-time resource usage and per-function metrics (\eg cold-start time, execution duration, resource demand distribution). Load prediction uses a basic FFT-based method implemented with SciPy~\cite{scipy}.
