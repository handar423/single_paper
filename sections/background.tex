...
Figure~\ref{fig:ratio_stability_scatter} further examines how CPU and memory usage co-vary within individual functions as request inputs change.\footnote{Benchmark names are abbreviated in the figure for readability; their full identifiers are listed in Table~\ref{tab:setup}.} Each point represents the mean usage for one input, and the line is a per-function linear fit. The distinct slopes across functions reinforce the need to account for function-specific resource profiles, while deviations from the fitted lines capture input-dependent variation within a function.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_ratio_stability_grid.pdf} \\ [-10pt]
  \caption{CPU and memory usage across request inputs for nine serverless functions. Each line shows a per-function linear fit.}
